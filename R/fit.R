#' @title fitData
#' @description Fit single cell gene expression data along pseudotime with flexible smoothing parameters
#' @details This function fits gene expression trajectories using a VGAM tobit model
#' with smoothing spline on pseudotime. Model parameters such as degrees of freedom,
#' tobit lower bound, and maximum iterations can be adjusted.
#' @param data a single cell expression matrix with rows as genes and columns as cells.
#' @param df degrees of freedom for spline smoothing (default = 2).
#' @param tobit_lower lower bound for tobit model (default = 0.1).
#' @param maxit maximum number of iterations for model fitting (default = 50).
#' @param expr.cut cutoff of lowest expression.
#' @param expr.cut.rate cutoff proportion of cells above expression threshold.
#' @param pseudo.time cells ranked according to pseudotime.
#' @param p.adjust.method method for multiple hypothesis test.
#' @param pt numeric pseudotime index used in model fitting.
...
#' @return  A list containing fitted models, scaled fitted expression matrix,
#' raw p-values and adjusted q-values.
#' @author Zhicheng Ji, Changxin Wan, Beijie Ji
#' @export fitData
#' @importFrom VGAM vgam tobit vgam.control lrtest s
#' @importFrom parallel mclapply
#' @importFrom stats p.adjust
fitData <- function(data, expr.cut=0.1, expr.cut.rate=0.05, pseudo.time=colnames(data), p.adjust.method="BH", pt=1:ncol(data), new_data=data.frame(pt=seq(1, ncol(data))), verbose=TRUE, mc.cores=1){
