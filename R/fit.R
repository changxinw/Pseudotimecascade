...
#' @export fitData
#' @importFrom VGAM vgam tobit vgam.control lrtest
...
fitData <- function(data, pt=1:ncol(data), expr.cut=0.1, expr.cut.rate=0.05, pseudo.time=colnames(data), p.adjust.method="BH", new_data=data.frame(pt=seq(1, ncol(data))), verbose=TRUE, mc.cores=1){
  ...
  #   pt <- 1:ncol(data)
  if (mc.cores > 1) {
    model <- parallel::mclapply(1:nrow(data), function(igene){
      if (igene %% 100 == 0 && verbose){
        message(paste0(igene, " genes processed!"))
      }
      VGAM::vgam(data[igene, ] ~ s(pt, df=2), family=VGAM::tobit(Lower=0.1, type.fitted = "mean.obs"), control=VGAM::vgam.control(maxit=50))
    }, mc.cores=mc.cores)
  }
  else {
  ...
  if (mc.cores > 1){
    pval_data <- parallel::mclapply(names(model), function(sg) {
      VGAM::lrtest(model[[sg]])@Body[2,5]
    }, mc.cores = mc.cores)
  } else {
  ...
  qval_data <- stats::p.adjust(unlist(pval_data), method = p.adjust.method)
